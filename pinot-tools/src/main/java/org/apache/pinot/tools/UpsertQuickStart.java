...
public class UpsertQuickStart {
  ...
  public void execute()
    ...
    File schemaFile = new File(quickStartDataDir, "upsert_meetupRsvp_schema.json");
    File tableConfigFile = new File(quickStartDataDir, "upsert_meetupRsvp_realtime_table_config.json");

    ClassLoader classLoader = Quickstart.class.getClassLoader();
    URL resource = classLoader.getResource("sample_data/upsert_meetupRsvp_schema.json");
    Preconditions.checkNotNull(resource);
    FileUtils.copyURLToFile(resource, schemaFile);
    resource = classLoader.getResource("sample_data/upsert_meetupRsvp_realtime_table_config.json");
    Preconditions.checkNotNull(resource);
    ...
    final Properties kafkaProperties = KafkaStarterUtils.getDefaultKafkaConfiguration();
    ...
    final KafkaServerStartable kafkaStarter = KafkaStarterUtils
        .startServer(KafkaStarterUtils.DEFAULT_KAFKA_PORT, KafkaStarterUtils.DEFAULT_BROKER_ID,
            KafkaStarterUtils.DEFAULT_ZK_STR, kafkaProperties);
